...
class FCM:
    ...
    def __init__(self, iterations, structure, activation_level, flt, number_used=False):
        ...
        self.model : dict[nx.DiGraph] = self.build_model(structure, activation_level)


    def build_model(self, structure, activation_level):
        "Build the FCM model"
        graph_list = {}
        self.desc_graphs = {}
        ...
        nodes = copy.deepcopy(structure['nodes'])
        for node in nodes:
            node.pop('destination_of', None)
        ...
        if not any(node_id not in self.idx_to_remove for node_id in intermediate):
    ...
    def activation_value(self, value):
        if isinstance(value, str):
            if value not in self.al_flt.linguistic_terms:
                raise ValueError(f"Unknown activation term: {value}")
            value = self.al_flt.get_value(value)
        value = float(value)
    ...
    def run_fcm(self, threshold=0.001):
        ...
        for key in self.model.keys():
            ...
            for n in G.nodes:
                G.nodes[n]['attr_dict']['value'] = G.nodes[n]['attr_dict']['value'][:t+1]
    ...
    def papageorgiou_alg_graph(graph, key_node, start_iter, end_iter, lambda_value, threshold=0.001):
        ...
        for t in range(start_iter,end_iter):    #for each iteration
            ...
            # Complete the synchronous update before checking convergence.
            if abs(G.nodes[key_node]['attr_dict']['value'][t] - G.nodes[key_node]['attr_dict']['value'][t-1]) < threshold:
                return G, t
...
if __name__ == "__main__":
    ...
    print(f"Algorithm: Papageorgiou, Iterations: {iterations}")
    fcm_obj = FCM(iterations, structure, activation_level, flt, number_used)
    ...
    with open(f'final_al2.json', 'w') as outfile:
